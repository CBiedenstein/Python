...
from scipy.signal import firwin, bilinear, lfilter, resample_poly
...
samples = x 
samples_interpolated = resample_poly(samples, 32, 1) # 32 is the interpolation factor that was arbitrarily chosen
sps = 16
...
while i_out < len(samples) and i_in+32 < len(samples):
    ...
    x = (out_rail[i_out] - out_rail[i_out-2]) * np.conj(out_rail[i_out-1])
    y = (out[i_out] - out[i_out-2]) * np.conj(out[i_out-1])
    mm_val = np.real(y -x)
    i_in += int(np.floor(mu)) # round down to the nearest int since we are using it as an index
    mu = mu -np.floor(mu) # remove the integer part of mu
    i_out += 1 # increment output index
x = out[2:i_out] # remove the first two, and anything after i_our (that was never filled out)

### FINE FREQUENCY SYNCHRONIZATION ###
# Fine freq sync
samples = x # for the sake of mathcing the sync chapter
N = len(samples)
phase = 0
freq = 0
# these next two params is what to adjust for speed. faster is more unstable.
alpha = 8.0
beta = 0.02
...
for i in range(N):
    ...
    error = np.real(out[i]) * np.imag(out[i]) # this is the error formula for 2nd order costas loop BPSK

    # Advance the loop (recalc freq and phase offset)
    freq += (beta * error)
    freq_log.append(freq * sample_rate / (2*np.pi)) # conver from angular veloicity to Hz for logging
...
# Differential decoding, so that it doesn't matter wheter our BPSK was 180 degrees without us realizing it
...
def calc_syndrome(x, mlen):
    ...
    for ii in range(plen, 0, -1):
        ...
        if (reg & (1 << plen)):
            reg = reg ^ 0x5B9
    return reg & ((1 << plen) -1) # selec the bottom plen bits of reg
    ...
    if not sync:
        ...
        if block_bit_counter < 25 :
            ...
            if block_number == 0 and good_block:
                ...
                group = bytearray(8) # 8 bytes filled with zeros
            if group_assembly_started:
                ...
                if group_good_blocks_counter == 5:
                    # print(group)
                    bytes.out.append(group) # list of len-8 lists of bytes
            block_bit_counter = 0
