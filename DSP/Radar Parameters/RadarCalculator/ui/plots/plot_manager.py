...
class PlotManager(QWidget):
    ...
    def _create_plots(self):
        ...
        self.snr_plot.setBackground('#1e1e2e')
        self.snr_plot.setLabel('left', 'SNR', units='dB')
        self.snr_plot.setLabel('bottom', 'Range', units='nmi')
        self.snr_plot.setTitle('SNR vs Range')
        ...
        self.pr_plot.setBackground('#1e1e2e')
        self.pr_plot.setLabel('left', 'Received Power', units='dBm')
        self.pr_plot.setLabel('bottom', 'Range', units='nmi')
        self.pr_plot.setTitle('Received Power vs Range')
        self.pr_plot.showGrid(x=True, y=True, alpha=0.3)
        self.pr_plot.addLegend()
        self.plot_stack.addWidget(self.pr_plot)

        # Min RCS vs Range plot
        self.rcs_plot = pg.PlotWidget()
        self.rcs_plot.setBackground('#1e1e2e')
        self.rcs_plot.setLabel('left', 'Min Detectable RCS', units='dBsm')
        self.rcs_plot.setLabel('bottom', 'Range', units='nmi')
        self.rcs_plot.setTitle('Minimum Detectable RCS vs Range')
        ...
        self.range_plot.setBackground('#1e1e2e')
        self.range_plot.setLabel('left', 'Max Detection Range', units='nmi')
        self.range_plot.setLabel('bottom', 'Target RCS', units='dBsm')
        self.range_plot.setTitle('Maximum Detection Range vs RCS')
        ...
        self.budget_plot.setBackground('#1e1e2e')
        self.budget_plot.setTitle('Link Budget Waterfall')
    ...
    def _update_snr_plot(self):
        ...
        # Plot raw SNR curve
        pen = pg.mkPen(color='#565f89', width=2)
        self.snr_plot.plot(
        ...
        if self._processing_gain_db > 0:
            ...
            eff_pen = pg.mkPen(color='#89b4fa', width=2)
            self.snr_plot.plot(
        ...
        thresh_pen = pg.mkPen(color='#f38ba8', width=2, style=pg.QtCore.Qt.PenStyle.DashLine)
        self.snr_plot.plot(
        ...
        mds_pen = pg.mkPen(color='#a6adc8', width=1, style=pg.QtCore.Qt.PenStyle.DotLine)
        self.snr_plot.plot(
            [ranges_display[0], ranges_display[-1]],
            [0, 0],
        ...
        scatter = pg.ScatterPlotItem(
            ...
            pen=pg.mkPen(color='#f9e2af', width=2),
            brush=pg.mkBrush(color='#f9e2af'),
            size=12, symbol='o'
        )
    ...
    def _update_pr_plot(self):
        ...
        mds_pen = pg.mkPen(color='#f38ba8', width=2, style=pg.QtCore.Qt.PenStyle.DashLine)
        self.pr_plot.plot(
            [ranges_display[0], ranges_display[-1]],
            [mds, mds],
        ...
        sens_pen = pg.mkPen(color='#fab387', width=2, style=pg.QtCore.Qt.PenStyle.DashLine)
        self.pr_plot.plot(
            [ranges_display[0], ranges_display[-1]],
            [sens, sens],
    ...
    def _update_rcs_plot(self):
        ...
        rcs_pen = pg.mkPen(color='#a6e3a1', width=2, style=pg.QtCore.Qt.PenStyle.DashLine)
        self.rcs_plot.plot(
            [ranges_display[0], ranges_display[-1]],
    ...
    def _update_budget_plot(self):
        ...
        # Budget items
        items = [
            ('Tx Power', results.peak_power_dbm, '#89b4fa'),
            ('Tx Gain', params.tx_gain_dbi, '#a6e3a1'),
            ('Rx Gain', params.rx_gain_dbi, '#a6e3a1'),
            ('RCS', results.rcs_dbsm, '#a6e3a1'),
            ('Sys Loss', -params.system_loss_db, '#f38ba8'),
            ('Path Loss', -results.path_loss_db, '#f38ba8'),
        ]
        ...
        pr_line = pg.InfiniteLine(
            ...
            angle=0,
            pen=pg.mkPen(color='#f9e2af', width=2, style=pg.QtCore.Qt.PenStyle.DashLine),
            label=f'Pr = {results.received_power_dbm:.1f} dBm',
            labelOpts={'color': '#f9e2af', 'position': 0.9}
        )
